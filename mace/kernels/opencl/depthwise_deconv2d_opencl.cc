...
#include "mace/kernels/depthwise_deconv2d.h"
...
MaceStatus DepthwiseDeconv2dOpencl(cl::Kernel *kernel,
                          const Tensor *input,
                          const Tensor *filter,
                          const Tensor *bias,
                          const int stride,
  ...
  const index_t width_blocks = width;
  ...
  if (kernel->get() == nullptr) {
    ...
    std::string opencl_file = "\"" MACE_DIR "/mace/kernels/opencl/cl/depthwise_deconv2d.cl\"";
    built_options.emplace("-s " + opencl_file);
#endif

    *kernel = runtime->BuildKernel("depthwise_deconv2d", kernel_name, built_options);
  ...
  std::string tuning_key =
      Concat("depthwise_deconv2d_opencl_kernel_", activation, output->dim(0),
...
template <typename T>
MaceStatus DepthwiseDeconv2dFunctor<DeviceType::GPU, T>::operator()(
    const Tensor *input,
    const Tensor *filter,
    const Tensor *bias,
    Tensor *output,
  ...
  if (!from_caffe_) {
    ...
    CalcDepthwiseDeconvOutputSize(input->shape().data(), filter->shape().data(),
  ...
  return DepthwiseDeconv2dOpencl(&kernel_, input, filter, bias, strides_[0],
                        paddings_.data(), activation_, relux_max_limit_,
                        DataTypeToEnum<T>::value, &input_shape_, output, future,
                        &kwg_size_, &kernel_error_);
}

template struct DepthwiseDeconv2dFunctor<DeviceType::GPU, float>;
template struct DepthwiseDeconv2dFunctor<DeviceType::GPU, half>;
