...
void Simple() {
  ...
  auto expected = CreateTensor<float>(
      {1, 6, 2, 1}, {-3.8543f, -3.8543f, -1.5125f, -1.5125f, 0.8291f, 0.8291f, 3.1708f,
                     3.1708f, 5.5125f, 5.5125f, 7.8543f, 7.8543f});
...
TEST_F(BatchNormOpTest, SimpleRandomOPENCL) {
  ...
  index_t channels = 3 + std::rand() % 25;
  index_t height = 64;
  index_t width = 64;
...
TEST_F(BatchNormOpTest, SimpleRandomHalfOPENCL) {
  ...
  static unsigned int seed = time(NULL);
  std::srand(seed);
  index_t batch = 1 + std::rand() % 10;
  index_t channels = 3 + std::rand() % 50;
  index_t height = 64;
  index_t width = 64;
