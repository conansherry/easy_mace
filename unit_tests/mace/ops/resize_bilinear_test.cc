...
void TestRandomResizeBilinear() {
  ...
  for (int round = 0; round < 10; ++round) {
    int batch = 1 + std::rand() % 5;
    int channels = 1 + std::rand() % 100;
    int height = 1 + std::rand() % 100;
    int width = 1 + std::rand() % 100;
    int in_height = 1 + std::rand() % 100;
    int in_width = 1 + std::rand() % 100;
    int align_corners = std::rand() % 1;
