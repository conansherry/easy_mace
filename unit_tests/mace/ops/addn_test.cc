...
void SimpleAdd3() {
  ...
  auto expected =
      CreateTensor<float>({1, 2, 3, 1}, {-0.000713f, 8, 12, 16, 20, 24});
...
void RandomTest() {
  ...
  for (int round = 0; round < 10; ++round) {
    ...
    index_t n = 1 + (std::rand() % 5);
    index_t h = 1 + (std::rand() % 100);
    index_t w = 1 + (std::rand() % 100);
    index_t c = 1 + (std::rand() % 32);
    int input_num = 2 + std::rand() % 3;
    // Construct graph
    OpsTestNet net;
