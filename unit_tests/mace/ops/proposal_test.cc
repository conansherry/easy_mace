...
TEST_F(ProposalOpTest, CPUSimple) {
  ...
  net.AddInputFromArray<DeviceType::CPU, float>("ImgInfo", {1, 1, 1, 3},
                                                {(float)img_height, (float)img_width, 2.f});

  // Run
  net.RunOp();

  auto expected_tensor = CreateTensor<float>({1, 1, 1, 5}, {0.f, 0.f, 0.f, 255.f, 255.f});

  ExpectTensorNear<float>(*expected_tensor, *net.GetTensor("Output"), 1e-5f);
}
